        ...
        <div className="relative max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-8">
              ...
              {PERIOD_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium transition ${
                    period === opt.value ? 'bg-white text-indigo-700' : 'text-indigo-200 hover:bg-white/10'
                  }`}
                >
                  {opt.label}
                </button>
            ...
            {subjects?.length > 0 && (
              <select
                ...
                className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-3 py-2 text-sm text-white focus:outline-none"
              >
          ...
          {!isEmpty && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Total Exams', value: overview.total_exams, sub: 'graded' },
                { label: 'Students', value: overview.total_students, sub: 'active' },
                { label: 'Avg Score', value: `${overview.average_percentage}%`, sub: 'class average' },
